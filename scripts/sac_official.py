"""ManiSkill SAC at 62ff3a5896b4d5b4cf0ac4c8d79afe600c9404a3.
Infrastructure-only fork: CPU simulation, exact budgets, JSONL and final evaluation.
Actor, critics, uniform replay and optimizer loop retain upstream semantics.
Upstream license: third_party/ManiSkill/LICENSE.
"""
import json
...
class Logger:
    def __init__(self, log_wandb=False, tensorboard: SummaryWriter = None) -> None:
        ...
        self.metrics = open(Path(tensorboard.log_dir) / "metrics.jsonl", "a", buffering=1)
    def add_scalar(self, tag, scalar_value, step):
        self.metrics.write(json.dumps(dict(tag=tag, value=float(scalar_value), env_steps=int(step))) + "\n")
        if self.log_wandb:
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    if args.num_envs < 1 or args.training_freq % args.num_envs or args.total_timesteps % args.num_envs:
        raise ValueError("training frequency and budget must be divisible by num_envs")
    if args.total_timesteps < 1 or args.training_freq < 1 or args.utd <= 0:
        raise ValueError("budget, training frequency and UTD must be positive")
    ...
    ####### Environment setup #######
    env_kwargs = dict(obs_mode="state", render_mode="rgb_array", sim_backend=args.sim_backend, render_backend="cpu")
    ...
    if args.capture_video or args.save_trajectory:
        eval_output_dir = f"{args.output_dir}/videos"
        if args.evaluate:
            eval_output_dir = f"{os.path.dirname(args.checkpoint)}/test_videos"
        print(f"Saving eval trajectories/videos to {eval_output_dir}")
        if args.save_train_video_freq is not None:
            save_video_trigger = lambda x : (x // args.num_steps) % args.save_train_video_freq == 0
            envs = RecordEpisode(envs, output_dir=f"{args.output_dir}/train_videos", save_trajectory=False, save_video_trigger=save_video_trigger, max_steps_per_video=args.num_steps, video_fps=30)
    ...
    if not args.evaluate:
        ...
        writer = SummaryWriter(f"{args.output_dir}")
        writer.add_text(
            "hyperparameters",
    ...
    while global_step <= args.total_timesteps:
        if global_step == args.total_timesteps or (args.eval_freq > 0 and (global_step - args.training_freq) // args.eval_freq < global_step // args.eval_freq):
            # evaluate
            actor.eval()
            ...
            if args.save_model:
                model_path = f"{args.output_dir}/ckpt_{global_step}.pt"
                torch.save({
        ...
        for local_step in range(min(args.steps_per_env, (args.total_timesteps-global_step)//args.num_envs)):
    ...
    if not args.evaluate and args.save_model:
        model_path = f"{args.output_dir}/final_ckpt.pt"
        torch.save({
            'actor': actor.state_dict(),
            'qf1': qf1_target.state_dict(),
            'qf2': qf2_target.state_dict(),
