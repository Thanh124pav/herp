...
from herp.allocator import AllocationConfig, allocate_budget, priority_distribution, robust_ranks
from herp.archive import RegionArchive
from herp.eval import evaluate_policy
from herp.gradient_signature import policy_gradient_signature, signature_parameters, empirical_fisher_diagonal
from herp.logging import CsvLogger
from herp.probe import default_obs_tensor, probe_region, reset_to_snapshot, elapsed_steps
...
class Args:
    ...
    probe_scale: float = 1.
    gamma_branch: float = .95
    lambda_dyn: float = 0.
    sigma_ema_tau: float = .9  # weight on the OLD estimate
    p_ema_tau: float = .9      # weight on the OLD estimate
    uniform_mix: float = .1
    staleness_mix: float = .05
    intrinsic_coef: float = .01
...
def parse_args(argv=None):
    ...
    parsed.pop('config')
    args = Args(**parsed)
    if args.method not in ('ppo', 'herp', 'herp_sigma', 'herp_p', 'rnd', 'disagreement', 'go_explore', 'plr'):
        parser.error('Unknown method')
    if args.p_estimator not in ('cosine', 'dot', 'fisher', 'occupancy'):
        parser.error('Unknown p estimator')
    if args.sigma_estimator not in ('pairwise', 'branch', 'return'):
        parser.error('Unknown sigma estimator')
    if not 0 <= args.allocated_frac < 1:
        parser.error('allocated-frac must be in [0, 1)')
    for name in ('total_timesteps', 'rollout_horizon', 'allocated_rollout_horizon', 'probe_horizon',
    ...
    for name in ('uniform_mix', 'staleness_mix', 'sigma_ema_tau', 'p_ema_tau', 'gamma', 'gae_lambda', 'lambda_dyn'):
...
def make_env(args):
    # Headless CPU rendering is needed on this WSL host; never override GPU choices.
    if args.render_backend == 'cpu':
        os.environ.setdefault('VK_ICD_FILENAMES', '/usr/share/vulkan/icd.d/lvp_icd.json')
    import mani_skill.envs  # noqa: F401
    return gym.make(args.env_id, obs_mode=args.obs_mode, reward_mode=args.reward_mode,
                    control_mode=args.control_mode, render_mode=None,
                    sim_backend=args.sim_backend, render_backend=args.render_backend)


def get_env_state(env):
    return copy.deepcopy(env.unwrapped.get_state_dict())
...
def collect_fragment(env, agent, args, horizon, source, regionizer=None,
    ...
    if start_snapshot is None:
        ...
        obs, _ = reset_to_snapshot(env, start_snapshot)
    ...
    for t in range(horizon):
        obs_t = default_obs_tensor(obs, args.device)
        if archive_states and t % args.archive_interval == 0:
            rid = regionizer.observe_snapshot(get_env_state(env), obs_t.cpu(), global_step+t,
                                              episode_id, return_so_far, elapsed_steps(env))
            recent.append(rid)
        with torch.no_grad():
            action, logprob, _, value = agent.get_action_and_value(obs_t[None])
            # Keep the sampled latent Gaussian action for exact PPO likelihood ratios.
            applied = clamp_action(action.squeeze(0), env)
            next_obs, reward, terminated, truncated, _ = env.step(applied.cpu().numpy())
            next_t = default_obs_tensor(next_obs, args.device)
            reward_f = float(torch.as_tensor(reward).mean())
            done = bool(terminated) or bool(truncated)
            # Time limits bootstrap from the FINAL observation, never a reset observation.
        ...
        for key, val in dict(obs=obs_t.cpu(), actions=action.squeeze(0).cpu(),
                             ...
                             dones=float(done), values=value.squeeze(0).cpu(), next_obs=next_t.cpu()).items():
            data[key].append(val)
    ...
    with torch.no_grad():
        next_value = agent.value(default_obs_tensor(obs, args.device)[None]).squeeze(0).cpu()
    batch['advantages'], batch['returns'] = compute_gae(batch['rewards'], batch['dones'], batch['values'],
                                                       next_value, args.gamma, args.gae_lambda)
    batch.update(steps=len(data['obs']), source=torch.full((len(data['obs']),), source, dtype=torch.long),
                 recent_regions=recent, episode_id=episode_id, next_value=next_value,
...
def concat_batches(batches):
    ...
    return {**{k: torch.cat([b[k] for b in batches]) for k in keys}, 'steps': sum(b['steps'] for b in batches)}


def signature(agent, batch, device):
    return policy_gradient_signature(agent, *(batch[k].to(device) for k in ('obs', 'actions', 'advantages')))
...
def update_region_scores(env, agent, args, archive, regionizer, recent_regions,
                         reference_gradient, step, max_probe_steps, reference_batch=None, fisher=None):
    use_sigma = args.method not in ('herp_p', 'go_explore', 'plr')
    use_gradient = args.method in ('herp', 'herp_p') and args.p_estimator != 'occupancy'
    ...
    occ = occupancy_scores(regionizer, reference_batch['obs']) if reference_batch is not None else torch.zeros(len(archive))
    for region in candidates:
        snapshot = archive.sample_snapshot(region)
        first = region.last_probed_step == 0
        if use_sigma:
            stats = probe_region(env, snapshot, agent, regionizer, args.num_probes, args.num_env_repeats,
                                 args.probe_horizon, args.probe_scale, args.gamma_branch, args.lambda_dyn,
                                 device=args.device, estimator=args.sigma_estimator, gamma=args.gamma)
            probe_steps += stats['steps']
            region.sigma_raw = stats['sigma']
            region.sigma_ema = region.sigma_raw if first else args.sigma_ema_tau * region.sigma_ema + (1-args.sigma_ema_tau)*region.sigma_raw
        else:
            region.sigma_ema = 1.
        if use_gradient or args.method == 'plr':
            batch = collect_fragment(env, agent, args, args.signature_horizon, ALLOCATED, start_snapshot=snapshot)
            signature_steps += batch['steps']
            batches.append(batch)  # genuine current-policy samples, also reused for PPO
            grad = signature(agent, batch, args.device)
...
def save_checkpoint(path, agent, optimizer, args, archive, regionizer, steps, update):
    payload = dict(agent=agent.state_dict(), optimizer=optimizer.state_dict(), config=asdict(args),
...
def main():
    ...
    (out_dir/'provenance.json').write_text(json.dumps({str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in sources}, indent=2))
    logger, region_logger = CsvLogger(out_dir/'metrics.csv'), CsvLogger(out_dir/'regions.csv')
    env, ref_env, eval_env = make_env(args), make_env(args), make_env(args)
    obs, _ = env.reset(seed=args.seed)
    obs_dim, action_dim = default_obs_tensor(obs).numel(), int(np.prod(env.action_space.shape))
    ...
    regionizer = OnlineRegionizer(obs_dim, archive, RegionizerConfig(region_radius=args.region_radius, max_regions=args.max_regions))
    intrinsic = (RND(obs_dim, action_dim) if args.method == 'rnd' else Disagreement(obs_dim, action_dim) if args.method == 'disagreement' else None)
    if intrinsic is not None:
        intrinsic.to(args.device)
        intrinsic_optimizer = torch.optim.Adam([p for p in intrinsic.parameters() if p.requires_grad], lr=args.learning_rate)
    ...
    initial = evaluate_policy(eval_env, agent, args.eval_episodes, args.device)
    ...
    while global_steps < args.total_timesteps:
        # Every update ends exactly at its interaction boundary, including evaluation checkpoints.
        ...
        normal = collect_fragment(env, agent, args, normal_horizon, NORMAL, regionizer, use_archive,
                                  global_step=global_steps)
        counts['normal_steps'] += normal['steps']
        batches = [normal]
        remaining = round_budget-normal['steps']
        reference_refreshed = False
        if needs_reference and remaining > 0 and (reference_batch is None or update % args.relevance_interval == 0):
            horizon = min(args.reference_horizon, remaining)
            reference_batch = collect_fragment(ref_env, agent, args, horizon, 3,
                                               reset_seed=1_000_000 + args.seed*10000 + update)
            # Reference data are excluded from PPO but fully charged to the training budget.
        ...
        if reference_refreshed:
            reference_gradient = signature(agent, reference_batch, args.device)
            if args.p_estimator == 'fisher':
                fisher = empirical_fisher_diagonal(agent, reference_batch['obs'].to(args.device), reference_batch['actions'].to(args.device))
        if use_archive and remaining > 0 and (not needs_reference or reference_gradient is not None) and update % args.scoring_interval == 0:
            candidates, probe_steps, signature_steps, signature_batches = update_region_scores(
                env, agent, args, archive, regionizer, normal['recent_regions'], reference_gradient,
                global_steps+normal['steps'], remaining, reference_batch, fisher)
        ...
        allocation = {}
        cfg = AllocationConfig(uniform_mix=args.uniform_mix, staleness_mix=args.staleness_mix,
                               current_step=global_steps+normal['steps'], beta=0. if args.method in ('herp_p','plr','go_explore') else 1.)
        ...
        while remaining > 0:
            if candidates:
                ...
                fragment = collect_fragment(env, agent, args, min(remaining,args.allocated_rollout_horizon),
                ...
                fragment = collect_fragment(env, agent, args, remaining, NORMAL, regionizer, use_archive,
                                            global_step=global_steps+round_budget-remaining)
                counts['normal_steps'] += fragment['steps']
            batches.append(fragment)
            remaining -= fragment['steps']
        intrinsic_loss, intrinsic_mean = 0., 0.
        if intrinsic is not None:
            ...
            with torch.no_grad():
                ...
                normal['advantages'],normal['returns'] = compute_gae(normal['rewards']+args.intrinsic_coef*bonus,
                    normal['dones'],normal['values'],normal['next_value'],args.gamma,args.gae_lambda)
        ...
        update+=1
        sigmas=[r.sigma_ema for r in archive] or [0.]
        ps=[r.p_ema for r in archive] or [0.]
        row=dict(global_env_steps=global_steps, update=update, wall_time=time.monotonic()-start_time,
                 ...
                 cumulative_eval_steps=eval_total,num_regions=len(archive),num_archived_states=sum(len(r.snapshots) for r in archive),
                 mean_sigma=float(np.mean(sigmas)),median_sigma=float(np.median(sigmas)),max_sigma=float(np.max(sigmas)),
                 mean_p=float(np.mean(ps)),median_p=float(np.median(ps)),
                 fraction_positive_alignment=float(np.mean([r.p_raw>0 for r in candidates])) if candidates else 0.,
                 priority_entropy=float(-(q*q.clamp_min(1e-12).log()).sum()),
                 allocation_histogram=json.dumps(allocation),
                 gradient_signature_norm_ref=float(reference_gradient.norm()) if reference_gradient is not None else 0.,
                 gradient_signature_norm_region=float(np.mean([r.gradient_norm for r in candidates])) if candidates else 0.,
                 intrinsic_loss=intrinsic_loss,intrinsic_mean=intrinsic_mean,**counts,
                 **{f'cumulative_{k}':v for k,v in cumulative.items()},**metrics)
        if global_steps >= next_eval or global_steps == args.total_timesteps:
            ...
            row.update(evaluation,cumulative_eval_steps=eval_total,wall_time=time.monotonic()-start_time)
            save_checkpoint(out_dir/f'checkpoint_{global_steps}.pt',agent,optimizer,args,archive,regionizer,global_steps,update)
            next_eval+=args.eval_interval
        for region in candidates:
            region_logger.log(dict(step=global_steps,env_id=args.env_id,region_id=region.region_id,
                                   sigma=region.sigma_ema,p=region.p_ema,alignment=region.p_raw,priority=region.priority,
                                   visit_count=region.count,last_seen=region.last_seen_step,last_probed=region.last_probed_step))
        logger.log(row)
        print(json.dumps(row),flush=True)
    (out_dir/'complete.json').write_text(json.dumps(dict(global_env_steps=global_steps,counts=cumulative,
                    wall_time=time.monotonic()-start_time,final_eval=evaluation),indent=2))
    for instance in (env,ref_env,eval_env):
        instance.close()
